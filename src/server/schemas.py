...
class OAuthCallbackRequest(BaseModel):
    """``POST /auth/callback`` body — PKCE code exchange.

    The ``code_verifier`` must be the original 43–128 character random string
    generated by the desktop client before the OAuth redirect.  The server
    forwards it to Google to prove possession (RFC 7636).
